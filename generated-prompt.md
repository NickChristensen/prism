...
- BarGraph: { title?: string, data: Array<{ label: string, value: number }>, annotation?: { value: number, label: string } } - Vertical bar chart for compact comparisons across labeled numeric values. Use for counts, rankings, categories, and short time ranges. Optionally include an annotation line with a value and label.
- LineGraph: { title?: string, data: Array<{ label: string, value: number }>, annotation?: { value: number, label: string } } - Line chart with points for numeric trends over ordered labels. Use for time series, progress, changes, and small trend summaries. Optionally include an annotation line with a value and label.
